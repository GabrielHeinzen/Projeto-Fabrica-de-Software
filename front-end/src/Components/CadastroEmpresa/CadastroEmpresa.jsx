  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (resposta.ok && dados.sucesso) {
        showToast('Empresa cadastrada com sucesso.', 'success', { title: 'Sucesso' });
        setFormData(initialForm); // limpa o formulário após sucesso
      } else {
        showToast(dados.mensagem || 'Erro ao cadastrar empresa', 'error', { title: 'Erro' });
      }
    } catch (erro) {
      console.log(erro);
    ...
    <div className="empresa-page">
      <aside className="empresa-sidebar">
        ...
        <nav className="empresa-nav">
          ...
          <button type="button" className="empresa-nav-item is-active" onClick={() => onNavigate && onNavigate('cadastro')}>Cadastro Empresa</button>
      ...
      <div className="empresa-content">
        ...
        <div className="empresa-grid">
          <form id="empresa-form" className="empresa-card empresa-card--form" onSubmit={handleSubmit}>
            ...
            <label className="empresa-field">
              ...
              <select id="regime-tributario" value={formData.regimeTributario} onChange={handleChange('regimeTributario')} required>
            ...
            {/* Grupos de radio buttons para perguntas Sim/Não */}
            <div className="empresa-toggle-group">
              <span>Possui funcionarios? *</span>
              <div className="empresa-toggle-options">
                <label className={`empresa-toggle-option ${formData.possuiFuncionarios === 'sim' ? 'is-selected' : ''}`}>
                  <input type="radio" name="possuiFuncionarios" value="sim" checked={formData.possuiFuncionarios === 'sim'} onChange={handleChange('possuiFuncionarios')} required />
                  Sim
                </label>
                <label className={`empresa-toggle-option ${formData.possuiFuncionarios === 'nao' ? 'is-selected' : ''}`}>
                  <input type="radio" name="possuiFuncionarios" value="nao" checked={formData.possuiFuncionarios === 'nao'} onChange={handleChange('possuiFuncionarios')} />
                  Nao
                </label>
              </div>
            </div>
            ...
            <div className="empresa-toggle-group">
              ...
              <div className="empresa-toggle-options">
                <label className={`empresa-toggle-option ${formData.possuiNotasVenda === 'sim' ? 'is-selected' : ''}`}>
                  <input type="radio" name="possuiNotasVenda" value="sim" checked={formData.possuiNotasVenda === 'sim'} onChange={handleChange('possuiNotasVenda')} required />
                  Sim
                </label>
                <label className={`empresa-toggle-option ${formData.possuiNotasVenda === 'nao' ? 'is-selected' : ''}`}>
                  <input type="radio" name="possuiNotasVenda" value="nao" checked={formData.possuiNotasVenda === 'nao'} onChange={handleChange('possuiNotasVenda')} />
                  Nao
                </label>
              </div>
            </div>
            ...
            <div className="empresa-toggle-group">
              ...
              <div className="empresa-toggle-options">
                <label className={`empresa-toggle-option ${formData.prestaServicos === 'sim' ? 'is-selected' : ''}`}>
                  <input type="radio" name="prestaServicos" value="sim" checked={formData.prestaServicos === 'sim'} onChange={handleChange('prestaServicos')} required />
                  Sim
                </label>
                <label className={`empresa-toggle-option ${formData.prestaServicos === 'nao' ? 'is-selected' : ''}`}>
                  <input type="radio" name="prestaServicos" value="nao" checked={formData.prestaServicos === 'nao'} onChange={handleChange('prestaServicos')} />
                  Nao
                </label>
              </div>
            </div>
            ...
            <div className="empresa-actions">
              {/* Botão desabilitado durante o envio para evitar requisições duplicadas */}
