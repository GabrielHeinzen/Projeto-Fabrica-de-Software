...
function Documentos({ userName = "Usuario", onLogout, onNavigate }) {
  ...
  const [permiteSemAnexo, setPermiteSemAnexo] = useState(false);
  ...
  const [editPermiteSemAnexo, setEditPermiteSemAnexo] = useState(false);
  ...
  const carregarDocumentos = useCallback(async () => {
    ...
    try {
      ...
      const documentosFormatados = Array.isArray(dados)
        ? dados.map((doc) => ({
            id: doc.id,
            nome: doc.nome,
            validade: doc.dia_limite_envio,
            periodicidade: doc.periodicidade,
            permite_sem_anexo: Number(doc.permite_sem_anexo) === 1,
          }))
        : [];
    ...
    try {
      ...
      const resposta = await fetch(`${apiBaseUrl}/documentos`, {
        ...
        body: JSON.stringify({
          ...
          periodicidade,
          permite_sem_anexo: permiteSemAnexo,
        }),
      });
  ...
  const handleEditRequest = (doc) => {
    ...
    setEditPermiteSemAnexo(Boolean(doc.permite_sem_anexo));
  };
  ...
  const handleEditConfirm = async () => {
    ...
    try {
      ...
      const resposta = await fetch(
        `${apiBaseUrl}/documentos/${documentoParaEditar.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
          ...
          body: JSON.stringify({
            ...
            permite_sem_anexo: editPermiteSemAnexo,
          }),
        },
      );
      ...
      if (resposta.ok && dados.sucesso !== false) {
        ...
        showToast(dados.mensagem || "Erro ao atualizar documento.", "error", {
          title: "Erro",
        });
      }
    } catch (erro) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const apiBaseUrl = getApiBaseUrl();
      const resposta = await fetch(
        `${apiBaseUrl}/documentos/${documentoParaExcluir.id}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        },
    ...
    <div className="empresa-page">
      ...
      {documentoParaEditar && (
        <div className="empresa-modal" role="dialog" aria-modal="true">
          ...
          <div
            className="empresa-modal__content empresa-modal__content--edit"
            role="document"
          >
            <div
            ...
            <div className="empresa-modal__fields">
              ...
              <label className="empresa-field">
                ...
                <select
                  ...
                  onChange={(e) => setEditPeriodicidade(e.target.value)}
                >
                  <option value="UNICO">Único</option>
                  <option value="MENSAL">Mensal</option>
                  <option value="TRIMESTRAL">Trimestral</option>
      ...
      {documentoParaExcluir && (
        <div className="empresa-modal" role="dialog" aria-modal="true">
          ...
          <div className="empresa-modal__content" role="document">
            <div className="empresa-modal__icon" aria-hidden="true">
              !
            </div>
            <div className="empresa-modal__text">
