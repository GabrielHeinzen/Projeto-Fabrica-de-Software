...
function AnexoDocumentos({ userName = "Usuario", onLogout, onNavigate }) {
  ...
  const [documentosSelecionados, setDocumentosSelecionados] = useState({}); // { idDocumento: File }
  ...
  //função de marcar/desmarcar documento para envio
  const alternarDocumentoSelecionado = (idDocumento) => {
          ...
          <section className="empresa-card">
            ...
            {!empresaSelecionada ? (
              ...
              <>
                <p>
                  Empresa selecionada:
                  <strong> {empresaSelecionada.razao_social}</strong>
                </p>
                ...
                <ul className="empresa-doc-list">
                  {documentos.map((doc) => (
                    <li
                      key={doc.id}
                      className={
                        documentosSelecionados[doc.id]
                          ? "empresa-documento empresa-documento--selecionado"
                          : "empresa-documento"
                      }
                    >
                      <div className="empresa-doc-checkbox">
                        <input
                          type="checkbox"
                          checked={!!documentosSelecionados[doc.id]}
                          onChange={() => alternarDocumentoSelecionado(doc.id)}
                        />
                      </div>
                ...
                <div className="empresa-actions" style={{ marginTop: "20px" }}>
                  ...
                  >
                    {quantidadeSelecionados === 0
                      ...
                      : `Enviar selecionados (${quantidadeSelecionados})`}
                  </button>
                </div>
              </>
            )}
